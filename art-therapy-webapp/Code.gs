const APP_TITLE = '미술치료 일지 및 사진대장';
const ROOT_FOLDER_NAME = '미술치료 일지 웹앱';
const JOURNAL_SHEET = 'Journals';
const OPTION_SHEET = 'Options';
const JOURNAL_HEADERS = [
  'id', 'title', 'journalDate', 'place', 'programName', 'therapist', 'photoDate',
  'media', 'attendeesJson', 'sessionRowsJson', 'photoIdsJson', 'diaryPdfId',
  'ledgerPdfId', 'submissionPdfId', 'createdAt', 'updatedAt'
];
...
function saveJournal(payload) {
  ...
  const existingPhotoIds = old ? parseJson_(old.photoIdsJson, []) : [];
  const photoIds = savePhotos_(id, payload.photos || [], existingPhotoIds);
  ...
  row.diaryPdfId = createDiaryPdf_(row);
  row.ledgerPdfId = createLedgerPdf_(row);
  upsertJournal_(row);
  rememberOptions_(row, payload.attendees || [], payload.sessionRows || []);
  return getJournal(id);
}

function createSubmissionPdf(id, includeDiary, includeLedger) {
  ...
  const body = [
    includeDiary ? diaryBody_(row) : '',
    includeDiary && includeLedger ? '<div class="page-break"></div>' : '',
    includeLedger ? ledgerBody_(row) : ''
  ].join('');
  row.submissionPdfId = savePdf_(wrapPdfHtml_(body), `${fileBase_(row)}_제출용.pdf`);
  row.updatedAt = new Date().toISOString();
  upsertJournal_(row);
  return { url: fileUrl_(row.submissionPdfId), journals: listJournals(), journal: getJournal(id) };
}

function deleteJournalParts(id, options) {
  const row = findJournal_(id);
  if (!row) throw new Error('일지를 찾을 수 없습니다.');
  if (options.diaryPdf) {
    deleteFile_(row.diaryPdfId);
  ...
  if (options.ledgerPdf) {
    deleteFile_(row.ledgerPdfId);
    row.ledgerPdfId = '';
  }
  if (options.submissionPdf) {
    deleteFile_(row.submissionPdfId);
    row.submissionPdfId = '';
  }
  if (options.originalPhotos) {
    parseJson_(row.photoIdsJson, []).forEach(deleteFile_);
    row.photoIdsJson = '[]';
  }
  if (options.data) {
    deleteJournalRow_(id);
  } else {
    row.updatedAt = new Date().toISOString();
    upsertJournal_(row);
  }
...
function addOption(type, value) {
  if (!OPTION_TYPES.includes(type)) throw new Error('잘못된 옵션입니다.');
  rememberOption_(type, value);
  return getOptions_();
}
...
function deleteOption(type, value) {
  ...
  for (let i = values.length - 1; i >= 1; i--) {
    if (values[i][0] === type && values[i][1] === value) sheet.deleteRow(i + 1);
  }
  return getOptions_();
}
...
function ensureStore_() {
  ...
  if (!spreadsheet) {
    spreadsheet = SpreadsheetApp.create(`${APP_TITLE} 데이터`);
...
function getSheet_(name) {
  const store = ensureStore_();
  return store.spreadsheet.getSheetByName(name);
}
...
function findJournal_(id) {
  return readObjects_(getSheet_(JOURNAL_SHEET)).find(row => row.id === id);
}
...
function savePhotos_(journalId, photos, existingIds) {
  ...
  photos.forEach((photo, index) => {
    ...
    const blob = dataUrlBlob_(photo.dataUrl, photo.name || `photo_${index + 1}.jpg`);
...
function createDiaryPdf_(row) {
  return savePdf_(wrapPdfHtml_(diaryBody_(row)), `${fileBase_(row)}_일지.pdf`);
}

function createLedgerPdf_(row) {
  return savePdf_(wrapPdfHtml_(ledgerBody_(row)), `${fileBase_(row)}_사진대장.pdf`);
}

function savePdf_(html, fileName) {
  const blob = Utilities.newBlob(html, MimeType.HTML, fileName.replace(/\.pdf$/i, '.html'))
    .getAs(MimeType.PDF)
    .setName(fileName);
...
function diaryBody_(row) {
  ...
  const attendeesHtml = attendees.map(item =>
    `${esc_(item.time)} - ${esc_(item.name)}`.replace(/^ - | - $/g, '')
  ).join('<br>');
  const sessionsHtml = sessionRows.map(item => `
    <div class="session-block">
      <strong>${esc_(item.name)}</strong>
      <div>${line_(item.content)}</div>
    </div>
  `).join('');

  return `
    <section class="diary-page">
      <h1>${esc_(row.title || '미술치료 일지')}</h1>
      <table class="diary-table">
        ...
        <tr><th>참 석 자</th><td colspan="3">${attendeesHtml}</td></tr>
        <tr><th>회기 내용</th><td colspan="3" class="session-cell">${sessionsHtml}</td></tr>
      </table>
    </section>`;
}
...
function ledgerBody_(row) {
  ...
  for (let page = 0; page < totalPages; page++) {
    ...
    pages.push(`
      <section class="ledger-page ${page ? 'page-break' : ''}">
        <table class="ledger-table"><tr>${cells[0]}${cells[1]}</tr><tr>${cells[2]}${cells[3]}</tr></table>
        <footer>미술치료 사진대장 / 활동일자 ${esc_(row.photoDate)} / ${page + 1}-${totalPages}</footer>
      </section>`);
  }
  return pages.join('');
}
...
function wrapPdfHtml_(body) {
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    body { font-family: "Malgun Gothic", Arial, sans-serif; color: #111; }
    h1 { text-align: center; font-size: 22px; margin: 0 0 12px; }
    table { border-collapse: collapse; width: 100%; }
    .diary-table th, .diary-table td { border: 1px solid #111; padding: 6px 8px; font-size: 12px; line-height: 1.55; vertical-align: top; }
    .diary-table th { width: 17%; background: #fff5c6; text-align: center; }
    .session-cell { min-height: 130mm; }
    .session-block + .session-block { border-top: 1px solid #999; margin-top: 8px; padding-top: 8px; }
    .session-block strong { display: block; margin-bottom: 3px; }
    .page-break { page-break-before: always; }
    .ledger-page { height: 261mm; position: relative; }
    .ledger-table { width: 100%; height: 239mm; border: 2px solid #111; table-layout: fixed; }
    .ledger-table td { width: 50%; height: 119.5mm; border: 1px solid #111; text-align: center; vertical-align: middle; }
    .ledger-table img { max-width: 96%; max-height: 116mm; object-fit: contain; }
    footer { position: absolute; left: 0; right: 0; bottom: 0; text-align: center; font-size: 11px; }
  </style></head><body>${body}</body></html>`;
}

function rememberOptions_(row, attendees, sessionRows) {
  rememberOption_('title', row.title);
  rememberOption_('place', row.place);
  rememberOption_('programName', row.programName);
  rememberOption_('therapist', row.therapist);
...
function rememberOption_(type, value) {
  ...
  if (!value) return;
  const sheet = getSheet_(OPTION_SHEET);
  const exists = readObjects_(sheet).some(row => row.type === type && row.value === value);
  if (!exists) sheet.appendRow([type, value, new Date().toISOString()]);
}
...
function getOptions_() {
  ...
  readObjects_(getSheet_(OPTION_SHEET)).forEach(row => {
    if (options[row.type] && row.value && !options[row.type].includes(row.value)) options[row.type].push(row.value);
  });
...
function getPhotoFolder_() {
  return ensureChildFolder_(ensureStore_().root, '원본사진');
}

function getPdfFolder_() {
  return ensureChildFolder_(ensureStore_().root, 'PDF');
}
...
function safeFolder_(id) {
  try { return id ? DriveApp.getFolderById(id) : null; } catch (e) { return null; }
}

function safeSpreadsheet_(id) {
  try { return id ? SpreadsheetApp.openById(id) : null; } catch (e) { return null; }
}

function dataUrlBlob_(dataUrl, fileName) {
  const parts = dataUrl.split(',');
  const type = (parts[0].match(/data:(.*?);base64/) || [])[1] || 'application/octet-stream';
  return Utilities.newBlob(Utilities.base64Decode(parts[1]), type, fileName);
}
...
function fileBase_(row) {
  return `${row.journalDate || row.photoDate || ''}_${row.title || '미술치료 일지'}`
    .replace(/[\\/:*?"<>|]/g, '_')
    .replace(/^_+/, '');
}
...
function line_(value) {
  return esc_(value).replace(/\n/g, '<br>');
}
